    ...
    def analyze_stock(self, ticker, sector):
        ...
        try:
            ...
            metrics = {
                ...
                'PEG Ratio': ratios.get('pegRatioTTM', 0),
                'DCF Value': dcf.get('dcf', 0),
                ...
                'MACD': technical_indicators.get('MACD', 0) if technical_indicators else 0,
                'Signal_Line': technical_indicators.get('Signal_Line', 0) if technical_indicators else 0,
                'Volume': technical_indicators.get('Volume_Current', 0) if technical_indicators else 0,
                '30D Volatility': volatility,
                'Beta': beta,
                'Debt/Equity': ratios.get('debtEquityRatioTTM', 0),
                'FCF': ratios.get('freeCashFlowPerShareTTM', 0) * profile.get('mktCap', 0) / profile.get('price', 1),
                'Dividend Yield': ratios.get('dividendYieldTTM', 0)
            }
            ...
            if getattr(st.session_state, 'analyze_clicked', False):
                with st.spinner('Performing comprehensive analysis...'):
                    ...
                    if results:  # Check if we have any results
                        ...
                        if 'Close' in df.columns and 'Daily_Return' in df.columns:
                            df.to_csv("classified_stocks.csv", index=False)
                        else:
                            st.warning("Close and Daily_Return columns are missing from the DataFrame.")
